  ...
  return (
    <main className="max-w-[1200px] mx-auto p-6 md:p-8 space-y-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Credit Balance Card */}
        <div className="p-6 rounded-2xl bg-bg-secondary border border-border-subtle flex flex-col justify-between space-y-4">
          <div>
            <span className="text-text-tertiary text-xs font-semibold uppercase tracking-wider block mb-1">Available Credits</span>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-5xl font-extrabold font-[family-name:var(--font-outfit)] text-brand-primary">
                {data?.credits}
              </span>
              <span className="text-text-secondary text-sm font-medium">credits</span>
            </div>
          </div>
        ...
        <div className="p-6 rounded-2xl bg-bg-secondary border border-border-subtle flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between">
              ...
              <span className={cn(
                "text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full shrink-0",
                data?.subscription.status === "active" ? "bg-success/15 text-success border border-success/20" : "bg-bg-tertiary text-text-tertiary border border-border-subtle"
              )}>
          ...
          <div className="mt-4 pt-4 border-t border-border-subtle/50 flex items-center justify-between">
            ...
            {data?.subscription.plan !== "free" && (
              <button
                ...
                className="text-xs text-brand-primary font-bold hover:underline focus-ring flex items-center gap-1"
              >
        ...
        <div className="p-6 rounded-2xl bg-bg-secondary border border-border-subtle flex flex-col justify-between space-y-4">
          <div>
            <span className="text-text-tertiary text-xs font-semibold uppercase tracking-wider block mb-1">Need more credits?</span>
            <div className="mt-2 flex items-center gap-2.5">
              <ShoppingBag className="w-6 h-6 text-brand-accent shrink-0" />
          ...
          <button
            ...
            className="w-full mt-4 py-2.5 bg-brand-primary hover:bg-brand-primary-hover text-white text-xs font-semibold rounded-lg transition-colors focus-ring"
          >
      ...
      {creditsSpent > 0 && data && (
        <section className="p-6 rounded-2xl bg-bg-secondary border border-border-subtle space-y-6">
          ...
          <div className="h-44 w-full flex items-end justify-between gap-3 pt-6 pb-2 border-b border-border-subtle/50 px-2">
      ...
      <section className="p-6 rounded-2xl bg-bg-secondary border border-border-subtle space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-border-subtle text-text-tertiary uppercase font-bold tracking-wider">
              ...
              {data?.transactions.length === 0 ? (
                ...
                data?.transactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-border-subtle/50 text-text-secondary hover:text-text-primary hover:bg-bg-tertiary/20">
                    <td className="py-3.5 px-3">{new Date(tx.createdAt).toLocaleDateString()}</td>
                    <td className="py-3.5 px-3 font-medium">{tx.description}</td>
                    ...
                    <td className={cn(
                      "py-3.5 px-3 text-right font-semibold",
